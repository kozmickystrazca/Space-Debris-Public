    ...
    for x in range(move_count):
        ...
        try:
            driver.find_element(
                By.ID, "right").click()
            time.sleep(delay)
            logging.info(test_name + ": moving right for " +
                         str(x + 1) + " times")
        except WebDriverException:
            check_for_optimalization(test_name, "continue")
            try:
    ...
    for x in range(move_count):
        ...
        try:
            driver.find_element(
                By.ID, "left").click()
            time.sleep(delay)
            logging.info(test_name + ": moving left for " +
                         str(x + 1) + " times")
        except WebDriverException:
            check_for_optimalization(test_name, "continue")
            try:
    ...
    for x in range(move_count):
        ...
        try:
            driver.find_element(
                By.ID, "up").click()
            time.sleep(delay)
            logging.info(test_name + ": moving up for " +
                         str(x + 1) + " times")
        except WebDriverException:
            check_for_optimalization(test_name, "continue")
            try:
    ...
    for x in range(move_count):
        ...
        try:
            driver.find_element(
                By.ID, "down").click()
            time.sleep(delay)
            logging.info(test_name + ": moving down for " +
                         str(x + 1) + " times")
        except WebDriverException:
            check_for_optimalization(test_name, "continue")
            try:
...
def choose_year(test_name, chosenYear):
    ...
    try:
        ...
        try:
            ...
            logging.warning(test_name + ": chosen year " +
                            "(" + str(chosenYear) + ")" + " not clickable!")
...
def get_year(test_name):
    ...
    try:
        ...
        logging.info(test_name + ": current year: " +
                     str(current_year.get_attribute("value")))
    except WebDriverException:
...
def open_menu(test_name):
    ...
    try:
        driver.find_element(
            By.CLASS_NAME, "menu-button").click()
...
def switch_checkbox(test_name, id):
    ...
    try:
        ...
        logging.info(
            test_name + ": switching checkbox with ID: '" + str(id) + "'")
    except WebDriverException:
        logging.error(test_name + ": checkbox with ID: '" +
                      str(id) + "' cannot be checked!")
...
def is_checked(test_name, id):
    ...
    try:
        ...
        if (is_checked):
            logging.info(test_name + ": checkbox with ID: '" +
                         str(id) + "' is CHECKED")
        else:
            logging.info(test_name + ": checkbox with ID: '" +
                         str(id) + "' is UNCHECKED")
    except WebDriverException:
        logging.error(test_name + ": checkbox with ID: '" + str(id) +
                      "' cannot return value! Check given ID parameter")
...
def switch_optimalization(test_name):
    ...
    try:
        ...
        logging.error(
            test_name + ": optimalization filter cannot be switched!")
...
def get_distance(test_name):
    ...
    try:
        distance = driver.find_element(
            By.CLASS_NAME, "distance")
        logging.info(
            test_name + ": current distance from Earth: " + str((re.findall(r'\d+', distance.text))[0]) + "km")
    except WebDriverException:
        logging.error(test_name + ": cannot get current distance from Earth")

    return (int)((re.findall(r'\d+', distance.text))[0])
...
def get_sat_count(test_name):
    ...
    try:
        sat_count = driver.find_element(
            By.CLASS_NAME, "display-count-objects")
        logging.info(
            test_name + ": current active satellites count: " + str((re.findall(r'\d+', sat_count.text))[-2]))
    except WebDriverException:
        logging.error(
            test_name + ": cannot get current active satellites count!")

    return (int)((re.findall(r'\d+', sat_count.text))[-2])
...
def get_deb_count(test_name):
    ...
    try:
        deb_count = driver.find_element(
            By.CLASS_NAME, "display-count-objects")
        logging.info(
            test_name + ": current space debris count: " + str((re.findall(r'\d+', deb_count.text))[-1]))
    except WebDriverException:
        logging.error(test_name + ": cannot get current space debris count!")

    return (int)((re.findall(r'\d+', deb_count.text))[-1])
...
def change_language(test_name, language):
    ...
    try:
        btn_change_language = driver.find_element(
            By.CLASS_NAME, "language-button-in-menu")

        if (not language == btn_change_language.text):
            logging.info(test_name + ": language (" +
            ...
            logging.error(
                test_name + ": incorrect language! Change attribute!")
...
def change_language_in_menu(test_name, language):
    ...
    try:
        btn_change_language = driver.find_element(
            By.CLASS_NAME, "language-button")

        if (not language == btn_change_language.text):
            logging.info(test_name + ": TITLE SCREEN language (" +
                         language + ") already chosen")

        elif (language == "EN" or language == "SK"):
            btn_change_language.click()
            logging.info(
                test_name + ": TITLE SCREEN changing language to: " + language)

        else:
            logging.error(
                test_name + ": TITLE SCREEN incorrect language! Change attribute!")
    except WebDriverException:
...
def check_for_optimalization(test_name, action):
    try:
        if (action == "optimize"):
            driver.find_element(
                By.ID, "button-optimalization").click()
            logging.info(test_name + ": enabling suggested optimalization")
        elif (action == "continue"):
            driver.find_element(
                By.ID, "button-continue").click()
            logging.info(test_name + ": ignoring suggested optimalization")
        else:
            logging.error(
                test_name + ": unknown action in suggested optimalization modal window! Change attribute!")

    except WebDriverException:
        logging.warning(
            test_name + ": modal window for suggested optimalization not available")
...
def test_result(test_name, has_passed):
    if (has_passed):
        logging.info(test_name + ": PASSED " + u'\u2713')
    else:   
        logging.info(test_name + ": FAILED " + u'\u2717')
